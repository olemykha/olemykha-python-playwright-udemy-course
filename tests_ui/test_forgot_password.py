...
from pom.login_page_elements import navigate_to_email_login, navigate_to_forgot_password
...
def test_reset_password_positive_scenario(navigate_to_the_site):
    ...
    navigate_to_forgot_password(page)
 ...
 #   expect(page.get_by_text("Reset password link was sent")).to_be_visible() site problems
...
def test_reset_password_non_existent_account(navigate_to_the_site):
    ...
    navigate_to_email_login(page)
    navigate_to_forgot_password(page)
    page.get_by_test_id("siteMembers.container").get_by_label("Email").fill("qweqweqweqwe@gmail.com")
    page.get_by_test_id("submit").get_by_test_id("buttonElement").click()
